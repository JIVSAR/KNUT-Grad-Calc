...
describe('다영역 동시 인정 (alsoCounts)', () => {
  it('전공이면서 K-융합: 전공계·K-융합 각각 +3, 총학점엔 1회', () => {
    const courses: Course[] = [
      course('자료구조', 3, 'majorElec', 'A0', { alsoCounts: ['kConvergeElec'] }),
    ]
    const r = evaluate(courses, SPEC)
    expect(area(r, 'majorTotal').earned).toBe(3)
    expect(area(r, 'kConvergeElec').earned).toBe(3)
    expect(area(r, 'total').earned).toBe(3)
    expect(r.totalEarned).toBe(3)
  })

  it('계획에서도 동시 인정 반영, 신청학점엔 1회', () => {
    const courses: Course[] = [
      course('딥러닝', 3, 'majorElec', '', {
        planned: true,
        semester: '4-1',
        alsoCounts: ['kConvergeElec'],
      }),
    ]
    const p = project(courses, SPEC)
    expect(p.rows.find((r) => r.id === 'majorTotal')!.cells[0].planned).toBe(3)
    expect(p.rows.find((r) => r.id === 'kConvergeElec')!.cells[0].planned).toBe(3)
    expect(p.termTotals[0].planned).toBe(3)
  })
})
...
describe('재수강 계획/수강중 — 명시적 retake 플래그는 새 학점 미반영', () => {
  ...
  it('동시인정 재수강(retaking) 계획은 전공·K-융합 영역 모두 변동 없음', () => {
    const courses: Course[] = [
      course('자료구조', 3, 'majorElec', 'C+', { semester: '2-1', alsoCounts: ['kConvergeElec'] }),
      course('자료구조', 3, 'majorElec', '', { planned: true, retaking: true, semester: '3-1', alsoCounts: ['kConvergeElec'] }),
    ]
    const p = project(courses, SPEC)
    expect(p.rows.find((r) => r.id === 'kConvergeElec')!.cells[0].planned).toBe(0)
